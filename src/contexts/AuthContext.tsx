...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async (email: string, password: string) => {
    try {
      ...
      setUser(userData);
      console.log("[Login] DEPOIS de setUser."); // Verifica se isAuthenticated (derivado) já é true aqui
      // -----------------------
  ...
  // Adicione logs ao useEffect também
   useEffect(() => {
     // ... início do useEffect ...
     getProfile()
       .then(userData => {
         console.log("[Effect] ANTES de setUser (via token).");
         setUser(userData);
         console.log("[Effect] DEPOIS de setUser (via token).");
       })
       .catch(error => {
         console.error("[Effect] Erro ao buscar perfil. ANTES de setUser(null).");
         localStorage.removeItem('token');
         delete api.defaults.headers.common['Authorization'];
         setUser(null);
         console.log("[Effect] Erro ao buscar perfil. DEPOIS de setUser(null).");
       })
 ...
 // src/contexts/AuthContext.tsx
  ...
  // ATENÇÃO: Removemos o setTimeout daqui e voltamos a lógica
  // para a LoginPage mostrar a toast baseada no sessionStorage.
  // Garanta que sua LoginPage.tsx ainda tem o useEffect que lê sessionStorage.
